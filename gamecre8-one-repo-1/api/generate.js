...
function generateGameFromPrompt(prompt) {
  ...
  return {
    meta: { title: `GameCre8 Placeholder — ${prompt || "Untitled"}`, promptUsed: prompt || "", instructions: "Use arrow keys to move. Collect all coins. Don’t touch lava." },
...
app.post("/", (req, res) => {
  ...
  res.status(200).json(generateGameFromPrompt(String(prompt || "")));
});
app.get("/", (_req, res) => res.status(200).json({ ok: true, route: "/api/generate" }));
