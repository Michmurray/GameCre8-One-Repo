// engine.js
// Minimal engine: draws platforms, coins, lava; player moves with arrows; win when coins=0.
...
export class TinyEngine {
  ...
  load(gameJson) {
    ...
    this.cv.width = world.width;
    this.cv.height = world.height;

    this.world = { ...world };
    this.player = { ...player, vx: 0, vy: 0, grounded: false };
    this.platforms = platforms.slice();
    this.coins = coins.slice();
    this.lava = lava.slice();
  ...
  reset() {
    this.world = { width: 800, height: 480, gravity: 0.7 };
    this.player = { x: 40, y: 40, w: 28, h: 28, speed: 3, jump: 12, vx: 0, vy: 0, grounded: false };
    this.platforms = [];
    this.coins = [];
    this.lava = [];
    this._raf = null;
  ...
  step() {
    ...
    for (const plat of this.platforms) {
      if (this.aabb(p, plat)) {
        ...
        if (p.vy < 0) { p.y = plat.y + plat.h; p.vy = 0; }
      }
    }

    p.x = Math.max(0, Math.min(this.world.width  - p.w, p.x));
    p.y = Math.max(0, Math.min(this.world.height - p.h, p.y));

    for (const l of this.lava) {
      if (this.aabb(p, l)) {
        p.x = 40; p.y = 40; p.vx = 0; p.vy = 0;
        this.status("Ouch! Lava. Try again.");
        break;
      }
    }
    ...
    for (let i = this.coins.length - 1; i >= 0; i--) {
      ...
      const cr = { x: c.x - c.size, y: c.y - c.size, w: c.size * 2, h: c.size * 2 };
      if (this.aabb(p, cr)) {
        this.coins.splice(i, 1);
        if (this.coins.length === 0) this.status("You collected all coins! 🎉");
      }
    }
  }
